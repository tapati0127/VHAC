...
from src.model.blink.crossencoder.crossencoder import CrossEncoderRanker, load_crossencoder
import logging

from src.model.blink.candidate_ranking import utils
from src.model.blink.biencoder import data_process as data
from src.model.blink.biencoder.zeshel_utils import DOC_PATH, WORLDS, world_to_id
from src.model.blink.common.optimizer import get_bert_optimizer
from src.model.blink.common.params import BlinkParser
...
def modify(context_input, candidate_input, max_context_length, max_cand_length, max_seq_length):
    new_input = []
    context_input = context_input.tolist()
    candidate_input = candidate_input.tolist()

    for i in range(len(context_input)):
        cur_input = context_input[i][:max_context_length]
        cur_candidate = candidate_input[i]
        mod_input = []
        for j in range(len(cur_candidate)):
            sample = cur_input + cur_candidate[j][1:][:max_cand_length]
            sample = sample[:max_seq_length]
            mod_input.append(sample)
        new_input.append(mod_input)
    return torch.LongTensor(new_input)
...
def evaluate(reranker, eval_dataloader, device, logger, context_length, zeshel=False, silent=True):
    ...
    for step, batch in enumerate(iter_):
        ...
        label_input = batch[1]
        with torch.no_grad():
            logits = reranker.score_candidate(context_input, context_len=context_length)
...
def main(params):
    ...
    cand_length = params["max_cand_length"]
    ...
    context_input = modify(context_input, candidate_input, context_length, cand_length, max_seq_length)
