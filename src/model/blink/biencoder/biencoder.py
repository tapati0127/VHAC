...
from src.model.blink.common.ranker_base import BertEncoder
from src.model.blink.common.optimizer import get_bert_optimizer
from src.model.blink.common.params import ENT_START_TAG, ENT_END_TAG, ENT_TITLE_TAG
...
class BiEncoderModule(torch.nn.Module):
    def __init__(self, params, tokenizer):
        ...
        if params.get('bert_base_dir'):
            ctxt_bert = AutoModel.from_pretrained(
                params['bert_base_dir'], local_files_only=False)
            cand_bert = AutoModel.from_pretrained(
                params['bert_base_dir'], local_files_only=False)
        else:
            ctxt_bert = AutoModel.from_pretrained(params["bert_model"], local_files_only=False)
            cand_bert = AutoModel.from_pretrained(params["bert_model"], local_files_only=False)
...
class BiEncoderRanker(torch.nn.Module):
    def __init__(self, params, shared=None):
        ...
        if model_path is not None:
            self.load_model(model_path, cpu=(self.device.type=='cpu'))
    ...
    def score_candidate(
        ...
        if cand_encs is not None:
            cand_encs = cand_encs.to(embedding_ctxt.device)
    ...
    def forward(self, context_input, cand_input, inbatch_target=None, label_input=None):
        flag = label_input is None
        scores = self.score_candidate(context_input, cand_input, flag)
        bs = scores.size(0)
        if label_input is None:
            # target = torch.LongTensor(torch.arange(bs))
            target = inbatch_target.to(self.device)
