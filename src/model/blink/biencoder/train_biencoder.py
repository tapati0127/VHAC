...
from src.model.blink.biencoder.biencoder import BiEncoderRanker, load_biencoder

import src.model.blink.candidate_ranking.utils as utils
import src.model.blink.biencoder.data_process as data
...
def evaluate(
    ...
    for step, batch in enumerate(iter_):
        ...
        context_input, candidate_input, orig_labels = batch
        with torch.no_grad():
            logits = reranker.score_candidate(context_input, candidate_input)
        ...
        # ).numpy()
        index_labels = create_labels(orig_labels.squeeze().tolist()).numpy()
        tmp_eval_accuracy, _ = utils.accuracy(logits, index_labels)
...
def main(params):
    ...
    for epoch_idx in trange(int(num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(iter_):
            ...
            # context_input, candidate_input, _, _ = batch
            context_input, candidate_input, orig_labels = batch
            target = create_labels(orig_labels.squeeze(1).tolist())
            loss, _ = reranker(context_input, candidate_input, inbatch_target=target)
